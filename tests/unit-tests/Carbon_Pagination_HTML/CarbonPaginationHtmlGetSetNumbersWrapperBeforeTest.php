<?php
/**
 * @group pagination
 * @group pagination_html
 */
class CarbonPaginationHtmlGetSetNumbersWrapperBeforeTest extends WP_UnitTestCase {

	public function setUp() {
		$paginationStub = $this->getMockForAbstractClass( 'Carbon_Pagination_HTML' );
		$this->pagination = $paginationStub;
	}

	public function tearDown() {
		unset( $this->pagination );
	}

	/**
	 * @covers Carbon_Pagination_HTML::get_numbers_wrapper_before
	 * @covers Carbon_Pagination_HTML::set_numbers_wrapper_before
	 */
	public function testGetSetNumbersWrapperAfter() {
		$html = '<div class="foo-pagination-numbers">';
		$this->pagination->set_numbers_wrapper_before( $html );
		$this->assertSame( $html, $this->pagination->get_numbers_wrapper_before() );
	}

}